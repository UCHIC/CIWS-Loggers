...
unsigned char get_volt(void);				// Function definitions
void int0_init(void);
...
unsigned char dataStorage[128];				// Global Array used to store records (Record Number, Pulse Count, Battery Voltage). Each record will be 4 bytes. When the Timer interrupt triggers and the Raspberry Pi is NOT reading the EEPROMS, then the contents of this array will be written to EEPROM.
...
unsigned char get_volt(void)
{
	unsigned char result;	
	start_conversion();				// Start an ADC conversion process
	while(!poll_interrupt_flag());	// Wait until the conversion is complete
	clear_interrupt_flag();			// Clear the interrupt flag
	result = read_ADCH();			// Store the conversion result

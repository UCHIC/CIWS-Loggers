...
if os.path.exists("LastRecord"):		# Determine the last record number to compute current record number in this run
	fh = open("LastRecord", "r")
	lastRecord = fh.read()
	fh.close()
...
filename = "test.csv"				# Define a path/filename to be used.
...
if os.path.exists(filename):			# If the file exists, 
	...
	fh.write("\nSiteName: ")
        ...
        fh.write("\nTime,Record,Pulses,Battery_Voltage,Disk_Space_Available (kB)\n")
...
# EEPROM loop
for i in range(numRecords):                     # loop reads an entire four byte record (sample)

...
# Get the meter readings at the time of the deployment passed as command line arguments
initialColdInMeterReading = sys.argv[1]
initialHotInMeterReading = sys.argv[2]
initialHotOutMeterReading = sys.argv[3]
...
print loggerName
coldInMeterReading = '"Cold supply meter reading at time of deployment: ' + initialColdInMeterReading + '\"'
print coldInMeterReading
hotInMeterReading = '"Hot supply meter reading at time of deployment: ' + initialHotInMeterReading + '\"'
print hotInMeterReading
hotOutMeterReading = '"Hot return meter reading at time of deployment: ' + initialHotOutMeterReading + '\"'
print hotOutMeterReading
...
dataHeader = '"Date\",\"RecordNumber\",\"coldInSensorVoltage\",\"coldInFlowRate\",\"coldInAvgFlowRate\",' \
             ...
             '\"coldInAvgFlowRate\",\"coldInIncrementalVolume\",\"coldInTotalizedVolume\",\"hotOutPulseCount\",' \
...
with open(outputFilePath + outputFileName, 'w') as outputFile:
    ...
    outputFile.write(coldInMeterReading + '\n')
    outputFile.write(hotInMeterReading + '\n')
    outputFile.write(hotOutMeterReading + '\n')
...
resistorValue = 200.0       # The value of the shunt resistor in ohms used for the 4-20 mA current loop measurement
...
def gpio_callback(GPIOPin):
    global pulseCount
    pulseCount += 1

# Set up pulse detection on the GPIO pin connected to the multi-jet meter
GPIO.add_event_detect(GPIOPin, GPIO.RISING, callback=gpio_callback, bouncetime=BOUNCE)
...
while True:
    ...
    if (currTime - previousTime) >= scanInterval:
        ...
        outputList = [now, sampleNum, coldInSensorVolts, coldInFlowRate, coldInAvgFlowRate, coldInRecordIntervalVolume,
                      coldInTotalizedVolume, hotInSensorVolts, hotInFlowRate, hotInAvgFlowRate,
                      hotInRecordIntervalVolume, hotInTotalizedVolume, hotOutPulseCountScan, hotOutFlowRate,
                      hotOutAvgFlowRate, hotOutRecordIntervalVolume, hotOutTotalizedVolume, netFlowRate, netAvgFlowRate,
                      netRecordIntervalVolume, netTotalizedVolume]
...
#       Allows pulse counting data to be displayed
#
